public class Car {
    ...
    public void drive(int km) throws Exception {
      if(km <= 0) throw new InvalidInputException(km);
      if(this.fuel < km) throw new  NotEnoughFuelException(km - this.fuel);
      if(!this.isRunning) throw new CarNotStartedException();
      this.fuel -= km;
    }
    ...
    public void fillCar(int fillFuel) throws Exception {
      if(fillFuel <= 0) throw new InvalidInputException(fillFuel);
      if(this.fuel + fillFuel > this.fuelCapacity) throw new FuelOverflowException(this.fuel + fillFuel - this.fuelCapacity);
